...
| `server/dance.cjs` + `ui/dance/` | **Dance Lab** at `/dance` — idol dance-video analysis. Pose estimation runs in the browser (MediaPipe); `ui/dance/analyze.mjs` (metrics/style), `beat.mjs` (audio tempo), `drills.mjs` (practice plan), `choreo.mjs` (8-count sheet, key-pose descriptions, repeated-phrase detection), `compare.mjs` (my cover vs the idol: audio/motion sync, strengths/weaknesses/fixes), `tracker.mjs` (multi-member tracking), `fancam.mjs` (per-member virtual camera + export), `synth.mjs` (demo group), `trends.mjs` (K-pop trend catalog + trend fit; refreshed via `agents/dance-trend-agent.md`). Reports land in `workspace/dance/`. Coach prompt: `agents/dance-coach-agent.md`. Tests: `npm test`. |
