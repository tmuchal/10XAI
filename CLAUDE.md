...
| `server/dance.cjs` + `ui/dance/` | **Dance Lab** at `/dance` — idol dance-video analysis. Pose estimation runs in the browser (MediaPipe); `ui/dance/analyze.mjs` (metrics/style), `beat.mjs` (audio tempo), `drills.mjs` (practice plan), `tracker.mjs` (multi-member tracking), `fancam.mjs` (per-member virtual camera + export), `synth.mjs` (demo group). Reports land in `workspace/dance/`. Coach prompt: `agents/dance-coach-agent.md`. Tests: `npm test`. |
