...
- **Every member, separately (group videos).** Each member gets a stable ID. Tracking uses position, velocity, body scale and an outfit-colour signature, and fragments are stitched back together after occlusions. Each member gets their own breakdown and practice plan. The **Members** tab compares styles side by side and shows a group-sync score (칼군무 degree). It also flags who is early or late on the count and names standouts (sharpest hits, most power, longest lines, most in sync).
- **Fancam (직캠) mode.** 🎥 Fancam 9:16 or 16:9 is a virtual camera that follows one member full-body, like a long-lens fancam: smooth pans, zoom that doesn't pump, and a minimap of the whole stage. **⬇ Export fancam** records it as a video file (with audio) for that member.
- **K-pop trends now.** A catalog of current choreography trends shown on the page, e.g. point-move hook challenges, relentless chorus footwork, predator drop & synced freeze, hip-isolation groove, wave canon and 32-step shuffle. Each trend has its signature move, a target profile on the same axes, example songs with sources (labelled *confirmed* or *reported*), and links to find practice videos. Every analysis (and every member) gets a **trend fit %** with the exact gaps (e.g. `Levels 5→85`). **🎯 Target** a trend and the practice plan adds trend cues and drills that close those gaps. **↻ Refresh trends** has your local `claude` CLI research the web (WebSearch) and replace the snapshot. The result is validated and saved to `workspace/dance/trends.json`; the prompt is `agents/dance-trend-agent.md`.
- **Count sheet (카운트 시트).** Built automatically from the beat grid, using audio beats or, without audio, a grid least-squares-fitted to the dancer's hits. For every 8-count it shows the key pose on each count: a mini stick figure plus plain words ("L arm overhead · wide stance"), hits, "&" accents and travel. Count "1" is placed where phrases repeat best. Repeated and mirrored phrases are detected (`A B A A′ C …`), so you get a learning order ("learn A, B → covers 70% of the song") and a *precision* score for how identically the idol repeats each phrase.
- **Practice mode.** Your webcam pose is compared to the dancer's, frame by frame, with a **👻 ghost** of the idol's pose drawn onto your own body. **👣 Step mode** freezes on each count's key pose until you match it (≥ 70), then plays to the next count. You get a live match score, a per-limb breakdown, and whether you're early or late. At the end you see *Reference vs You* numbers (energy, stops, reach, bounce, knee angle) and a per-phrase table showing where you rush or drag.
- **AI coach (optional).** The local `claude` CLI writes a brief in English or Korean from the measurements.

YouTube's embedded player can't be read frame by frame. To analyze a video, either upload a clip or, if `yt-dlp` is installed, fetch a local copy for personal study (this needs explicit confirmation). Videos and reports are stored in `workspace/dance/` (gitignored). Try **✨ Demo dancer** (a 4-member group where one member is late and one dances a different routine) to see the full flow without a video.
