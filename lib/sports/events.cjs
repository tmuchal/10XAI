...
//     lineups: { home: [{ player, minutes, position?, on?, off?, starter? }], away: [...] },
//     events:  [{ t, team: "home"|"away", player, type, outcome?, x?, y?, endX?, endY?, xg? }]
//   }
//
// Optional spatial fields use a 0–100 pitch from the acting team's perspective:
// x = 0 own goal line → 100 opponent goal line, y = 0 left touchline → 100 right.
// position is a line: GK | DF | MF | FW (detail, e.g. "Left Back", goes in role).
//
...
const SOCCER_TYPES = ["pass", "key_pass", "assist", "shot", "dribble", "tackle", "interception", "clearance", "save", "foul", "turnover", "card",
  "carry", "pressure", "recovery", "aerial", "block", "dribbled_past"];
...
function normalizeMatch(raw) {
  ...
  (Array.isArray(raw.events) ? raw.events : []).forEach((e, i) => {
    ...
    for (const k of ["x", "y", "endX", "endY", "xg", "confidence"]) {
      if (e[k] === "" || e[k] == null) continue;
      const v = Number(e[k]);
      if (Number.isFinite(v)) ev[k] = v;
    }
    events.push(ev);
  });
  ...
  for (const side of ["home", "away"]) {
    ...
    if (given) {
      for (const p of given) {
        ...
        const entry = { player: String(name).trim() };
        if (typeof o.on === "number" || typeof o.off === "number") {
          entry.on = typeof o.on === "number" ? o.on : 0;
          entry.off = typeof o.off === "number" ? o.off : DEFAULT_MINUTES[sport];
        }
        entry.minutes = typeof o.minutes === "number" ? o.minutes : entry.on != null ? Math.max(0, entry.off - entry.on) : DEFAULT_MINUTES[sport];
        const pos = normPosition(o.position);
        if (pos) entry.position = pos;
        if (o.role) entry.role = String(o.role);
        if (typeof o.starter === "boolean") entry.starter = o.starter;
        else if (entry.on != null) entry.starter = entry.on === 0;
        lineups[side].push(entry);
      }
    }
  ...
  if (!score && events.length) { score = derived; warnings.push("score derived from events"); }
...
// Accepts GK/DF/MF/FW or common role names ("Left Back", "Striker", "골키퍼").
function normPosition(p) {
  if (!p) return null;
  const s = String(p).trim();
  const up = s.toUpperCase();
  if (POSITIONS.includes(up)) return up;
  if (/goal ?keeper|^gk$|골키퍼/i.test(s)) return "GK";
  if (/back|defen[cs]e|defender|수비|^(cb|lb|rb|lwb|rwb)$/i.test(s)) return "DF";
  if (/midfield|^(cm|dm|am|cdm|cam|lm|rm)$|미드/i.test(s)) return "MF";
  if (/wing|forward|striker|^(st|cf|lw|rw|fw)$|공격|윙/i.test(s)) return "FW";
  return null;
}
...
module.exports = { TYPES, OUTCOMES, POSITIONS, DEFAULT_MINUTES, normalizeMatch, normPosition, parseEventsCsv, parseTime, scoreFromEvents };
