...
function predictMatch(history, fixture, opts = {}) {
  ...
  if (sport === "soccer") {
    const base = avgGoals(ms, sport) || P.baseGoals;
    const lh = base * Math.exp(P.goalK * d), la = base * Math.exp(-P.goalK * d);
    ...
    const tot = pH + pD + pA;
    out.probabilities = { home: r3(pH / tot), draw: r3(pD / tot), away: r3(pA / tot) };
    out.expectedGoals = { home: r2(lh), away: r2(la) };
    out.likelyScores = grid.sort((x, y) => y.p - x.p).slice(0, 5).map((g) => ({ score: g.score, p: r3(g.p / tot) }));
  } else {
...
function backtest(matches, sport, opts = {}) {
  ...
  for (let i = warmup; i < ms.length; i++) {
    ...
    const pred = predictMatch(hist, { sport, home: m.home, away: m.away, lineups: m.lineups }, opts);
    const diff = m.score.home - m.score.away;
  ...
  return { sport, n: rows.length, warmup, model, baseline,
...
module.exports = { predictMatch, backtest, buildElo, PARAMS, expectScore };
